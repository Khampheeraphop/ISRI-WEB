...
export function SelectFieldControl<T extends FieldValues>({ control, field }: SelectFieldControlProps<T>) {
  return <Controller control={control} name={field.name} render={({ field: controllerField, fieldState }) => <TextField {...controllerField} select label={field.label} required={field.required} error={Boolean(fieldState.error)} helperText={fieldState.error?.message} fullWidth>{field.options?.map((option) => <MenuItem key={option.value} value={option.value}>{option.label}</MenuItem>)}</TextField>} />;
}
