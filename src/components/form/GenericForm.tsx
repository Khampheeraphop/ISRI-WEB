...
import { useForm, useWatch, type DefaultValues, type FieldValues, type Resolver, type SubmitErrorHandler, type SubmitHandler } from "react-hook-form";
...
export function GenericForm<T extends FieldValues>({ fields, schema, defaultValues, submitLabel, onSubmit, onInvalid, onCancel, cancelLabel = "ยกเลิก", isSubmitting, columns = 1 }: GenericFormProps<T>) {
  ...
  return <Box component="form" onSubmit={handleSubmit(onSubmit, onInvalid)} sx={{ display: "grid", gap: 2, gridTemplateColumns: columns === 2 ? { xs: "1fr", md: "repeat(2, minmax(0, 1fr))" } : "1fr" }} noValidate>
