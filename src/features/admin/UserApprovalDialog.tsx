...
export function UserApprovalDialog({
  ...
  const submit = (approvalStatus: "approved" | "rejected") =>
    onSubmit({
      approvalStatus,
      role: approvalStatus === "approved" ? role : undefined,
      technicianSpecialties: role === "technician" ? specialties : [],
      note,
    });
  return (
    <Dialog
          ...
          <DialogContent dividers>
            <Stack spacing={2.5}>
              ...
              <Box>
                ...
                <Typography>
                  {user.requestedPosition ?? "ยังไม่ระบุ"}
                </Typography>
              </Box>
              {user.approvalStatus === "pending" && (
                <>
                  <FormControl fullWidth>
                    <InputLabel id="approval-role-label">
                      สิทธิ์การใช้งาน
                    </InputLabel>
                    <Select
              ...
              {user.approvalStatus !== "pending" && (
                <Alert
                  severity={
                    user.approvalStatus === "approved" ? "success" : "error"
                  }
                >
