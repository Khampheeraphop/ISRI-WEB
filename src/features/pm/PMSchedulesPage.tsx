    ...
    <Stack spacing={3}>
      <Box sx={{ display: "flex", justifyContent: "space-between", gap: 2, alignItems: { xs: "flex-start", sm: "center" }, flexDirection: { xs: "column", sm: "row" } }}>
        <Box>
          ...
          <Typography color="text.secondary" sx={{ mt: 0.5 }}>ติดตามรอบตรวจเช็คครุภัณฑ์และบันทึกผลการทำ PM</Typography>
        </Box>
        {isAdmin && <Button component={Link} to="/pm/new" variant="contained" startIcon={<AddOutlined />}>ตั้งรอบ PM</Button>}
      </Box>
      {schedules.isError && <Alert severity="error">ไม่สามารถโหลดแผน PM ได้</Alert>}
      {!schedules.data?.length && !schedules.isError && <MainCard><Typography color="text.secondary">ยังไม่มีแผน PM ในระบบ</Typography></MainCard>}
      <Stack spacing={2}>
          ...
          return (
            ...
            >
              <Stack direction={{ xs: "column", sm: "row" }} spacing={{ xs: 1, sm: 4 }}>
                <Stack direction="row" spacing={1} sx={{ alignItems: "center" }}><CalendarMonthOutlined color="primary" fontSize="small" /><Typography>ทำทุก {schedule.intervalMonths} เดือน</Typography></Stack>
